...
2. Create a timeout that `console.log`s the text **My name is {name}** only after there has been a 1 second delay since the last time the name was changed.
    * For example, if I change the name from **Nitin** to **Nit** and then to **Ni** without having more than 1 second between each name change the console should log nothing until 1 second after I finishing changing the name to **Ni** and then it will log **Ni**. If instead there as greater than 1 second delay between each change, it should log **Nit** and then **Ni**. Each of those logs would happen exactly 1 second after the name was changed.